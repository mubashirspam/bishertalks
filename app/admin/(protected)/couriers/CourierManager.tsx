...
export default function CourierManager({
  ...
  /** This courier's own declared parcel size, per book. Empty keeps the
   * measured default in lib/courier-sheet.ts's COURIER_DEFAULTS. */
  const [dims, setDims] = useState({
    weight: "", giftWrap: "", length: "", breadth: "", height: "",
  });
  ...
  const openSheet = (c: Courier) => {
    ...
    setDims({
      weight: c.config.weight_grams ?? "",
      giftWrap: c.config.gift_wrap_grams ?? "",
      length: c.config.length_cm ?? "",
      breadth: c.config.breadth_cm ?? "",
      height: c.config.height_cm ?? "",
    });
  };
  ...
  const saveSheet = async (c: Courier) => {
    ...
    const ok = await call("PATCH", {
      ...
      config: {
        ...
        weight_grams: dims.weight,
        gift_wrap_grams: dims.giftWrap,
            ...
            {couriers.map((c) => (
              ...
              {editing === c.id && (
                <div className="mt-4 rounded-xl border border-neutral-200 bg-neutral-50/70 p-4">
                  ...
                  <p className="text-xs font-medium text-neutral-700 mb-1 mt-5 pt-4 border-t border-neutral-200">
                    Weight &amp; dimensions, per book
                  </p>
                  <p className="text-[11px] text-neutral-500 mb-3">
                    What this courier is told the parcel weighs and measures —
                    the download sheet, the live send, and freight pricing all
                    use this number for {c.name}. Leave a field empty to use
                    the measured default (380 g, 25 × 15 cm, 2.0 cm per book).
                  </p>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        min="0"
                        value={dims.giftWrap}
                        onChange={(e) => setDims({ ...dims, giftWrap: e.target.value })}
                        placeholder="20"
                        className="w-full bg-white border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary-500 transition-colors"
                      />
                    </div>
                  ...
                  <p className="text-[11px] text-neutral-500 mt-1.5">
                    Length and breadth are the flat of the parcel and don&apos;t
                    grow with quantity; weight and height do, one book at a
                    time.
                  </p>
